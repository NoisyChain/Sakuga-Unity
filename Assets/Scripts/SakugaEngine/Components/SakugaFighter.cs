...
namespace SakugaEngine
{
    ...
    {
        [Header("Timers")]
        public FrameTimer HitStun;
        public FrameTimer HitStop;
        public FrameTimer  MoveBuffer;
        public FrameTimer  PushForce;
        public FrameTimer  HorizontalBounce;
        public FrameTimer  VerticalBounce;

        [Header("Lists")]
        public SpawnsList SpawnablesList;

        [Header("Extras")]
        public FighterProfile Profile;

        [HideInInspector] public int LayerSorting = -1;

        [HideInInspector] public bool SuperFlash;
        [HideInInspector] public bool CinematicState;
        ...
        public void SpawnablesSetup(Transform Parent, PhysicsWorld world)
        {
            ...
            for (int i = 0; i < Spawnables.Length; ++i)
            {
                ...
                {
                    GameObject temp = Instantiate(SpawnablesList.SpawnObjects[i].SpawnPrefab, Parent);
                    Spawnables[i][j] = temp.GetComponent<SakugaSpawnable>();
        ...
        public void VFXSetup(Transform Parent)
        {
            ...
            for (int i = 0; i < VFX.Length; ++i)
            {
                ...
                {
                    GameObject temp = Instantiate(VFXList.SpawnObjects[i].SpawnPrefab, Parent);
...
#region Push Force
        ...
        public void HitstunRecover()
        {
            ...
            if (Inputs.WasPressed(Inputs.CurrentHistory, Global.INPUT_ANY_BUTTON))
            {
                if (Inputs.IsBeingPressed(Inputs.CurrentHistory, Global.INPUT_LEFT))
                    ...
                    recoveryDirection = Global.SignInt(Body.FixedVelocity.x);
        ...
        public void CallBlockState(int blockType)
        {
            if (Body.IsOnGround)
            {
                if (IsCrouching())
                {
                    ...
                    Debug.Log(Animator.States[Stance.GetCurrentStance().CrouchBlockInitialState + blockType].StateName);
                }
                else
                {
                    ...
                    Debug.Log(Animator.States[Stance.GetCurrentStance().GroundBlockInitialState + blockType].StateName);
                }
            }
            else
            {
                ...
                Debug.Log(Animator.States[Stance.GetCurrentStance().AirBlockInitialState + blockType].StateName);
            }
        }

        public void SpawnSpawnable(int index, Vector2Int pos)
        {
        ...
        public void SpawnVFX(int index, Vector2Int pos)
        {
...
#region Damage functions
        ...
        public void ArmorHit(HitboxElement box)
        {
            //Armor hit
            var finalDamage = FighterVars.CalculateCompleteDamage(box.BaseDamage, GetOpponent().FighterVars.CurrentAttack);
            HitConfirm(box.OpponentSuperGaugeGain, (uint)box.ClashHitStopDuration, -1, -1, Vector2Int.zero);
            LayerSorting = -1;
...
#region Interface functions
            ...
            else
            {
                ...
                if (GetOpponent().IsBlocking() && !HitPosition)
                {
                    if (box.GuardCrush)
                    {
                        ...
                        Debug.Log("Fighter: Guard Crushed!");
                    }
                    else
                    {
        ...
        {
            HitDamage(box, true);
            if (box.HitEffectIndex >= 0 && contact != Vector2Int.zero)
            {
                SpawnVFX(box.HitEffectIndex, contact);
            }
            Debug.Log("Fighter: Traded!");
        }
        public void ThrowDamage(SakugaActor target, HitboxElement box, Vector2Int contact)
        {
            ...
            HitConfirm(0, finalHitstop, box.HitConfirmState, -1, Vector2Int.zero);
            Debug.Log("Fighter: Throw!");
        }
        ...
        public void CounterHit(SakugaActor target, HitboxElement box, Vector2Int contact)
        {
            ...
            GetOpponent().HitConfirm(box.OpponentSuperGaugeGain, (uint)box.OpponentHitStopDuration, -1, -1, Vector2Int.zero);
        }
